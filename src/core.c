...
void finalize(const argon2_context *context, argon2_instance_t *instance) {
    if (context != NULL && instance != NULL) {
        ...
        /* Hash the result */
        {
            ...
            aquablake2b_long(context->out, context->outlen, blockhash_bytes,
                         ARGON2_BLOCK_SIZE);
...
void fill_first_blocks(uint8_t *blockhash, const argon2_instance_t *instance) {
    ...
    for (l = 0; l < instance->lanes; ++l) {
        ...
        aquablake2b_long(blockhash_bytes, ARGON2_BLOCK_SIZE, blockhash,
...
void initial_hash(uint8_t *blockhash, argon2_context *context,
    ...
    aquablake2b_init(&BlakeHash, ARGON2_PREHASH_DIGEST_LENGTH);

    store32(&value, context->lanes);
    aquablake2b_update(&BlakeHash, (const uint8_t *)&value, sizeof(value));
    ...
    if (context->pwd != NULL) {
        aquablake2b_update(&BlakeHash, (const uint8_t *)context->pwd,
                       context->pwdlen);
    ...
    if (context->salt != NULL) {
        aquablake2b_update(&BlakeHash, (const uint8_t *)context->salt,
                       context->saltlen);
    }
    ...
    if (context->secret != NULL) {
        aquablake2b_update(&BlakeHash, (const uint8_t *)context->secret,
                       context->secretlen);
    ...
    if (context->ad != NULL) {
        aquablake2b_update(&BlakeHash, (const uint8_t *)context->ad,
                       context->adlen);
    }

    aquablake2b_final(&BlakeHash, blockhash, ARGON2_PREHASH_DIGEST_LENGTH);
}
